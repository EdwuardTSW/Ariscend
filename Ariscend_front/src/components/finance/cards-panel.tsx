...
import { currencyOptions, formatMoney, inputClass, labelClass } from "@/lib/finance-utils";
...
function emptyForm(currency = "MXN"): CardFormState {
  return { alias: "", issuer: "", type: "DEBIT", lastFourDigits: "", currency, creditLimit: "", openingBalance: "", closingDay: "", paymentDueDay: "" };
}
...
export function CardsPanel() {
  ...
  useEffect(() => {
    ...
    let active = true;
    Promise.all([financeApi.cards(selectedUser.id), financeApi.settings(selectedUser.id)]).then(async ([result, settings]) => {
      ...
      setForm((current) => current.alias ? current : { ...current, currency: settings.baseCurrency });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setForm(emptyForm(baseCurrency)); setModalOpen(false); setReloadKey((value) => value + 1);
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between gap-4"><div><h2 className="text-xl font-semibold">Tus tarjetas</h2><p className="mt-1 text-sm text-[#85878a]">Sólo guardamos alias y últimos cuatro dígitos.</p></div><div className="flex gap-2"><Button variant="ghost" size="icon" onClick={() => setShowAmounts((value) => !value)} aria-label="Mostrar u ocultar montos">{showAmounts ? <Eye className="size-4" /> : <EyeOff className="size-4" />}</Button><Button size="small" onClick={() => { setForm(emptyForm(baseCurrency)); setModalOpen(true); }}><Plus className="size-4" /> Agregar</Button></div></div>
      {activeCards.length === 0 ? <ObsidianCard className="flex min-h-64 flex-col items-center justify-center p-8 text-center"><WalletCards className="size-8 text-[#77797c]" /><h3 className="mt-4 text-lg font-semibold">Sin tarjetas registradas</h3><p className="mt-2 max-w-sm text-sm text-[#85878a]">Agrega una tarjeta de débito o crédito para calcular saldo, deuda y fechas.</p><Button className="mt-5" onClick={() => { setForm(emptyForm(baseCurrency)); setModalOpen(true); }}><Plus className="size-4" /> Agregar tarjeta</Button></ObsidianCard> : <div className="grid gap-4 lg:grid-cols-2">{activeCards.map((card) => {
      ...
      <FinanceModal open={modalOpen} onOpenChange={setModalOpen} title="Registrar tarjeta" description="Ariscend nunca solicitará el número completo, CVV o PIN.">
        <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
          ...
          <label className={labelClass}>Moneda<select value={form.currency} onChange={(event) => updateForm("currency", event.target.value)} className={`${inputClass} mt-2`}>{currencyOptions.map((currency) => <option key={currency.code} value={currency.code}>{currency.code} · {currency.label}</option>)}</select></label>
